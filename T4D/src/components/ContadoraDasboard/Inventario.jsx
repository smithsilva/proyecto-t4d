...
function Inventario() {
 ...
 const recargar = async () => {
  ...
  const nombreProveedor = (id) =>
    proveedores.find((p) => p.id_proveedor == id)?.nombre_proveedor || "—";
    ...
    <div className="p-4" style={{ margin: 0, backgroundColor: FONDO, minHeight: "100vh", width: "100%" }}>
      ...
      <div className="p-4 rounded-4 shadow-sm mb-4" style={{ backgroundColor: "#fffdf8", border: `1px solid ${DORADO_CLARO}` }}>
        ...
        <div className="mb-3 position-relative">
          ...
          <input type="text" className="form-control rounded-pill" style={{ paddingLeft: "36px", paddingTop: "10px", paddingBottom: "10px" }}
            placeholder="Buscar por producto, código o descripción..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} />
        </div>
      ...
      <div className="rounded-4 shadow-sm overflow-hidden" style={{ backgroundColor: "#fffdf8", border: `1px solid ${DORADO_CLARO}` }}>
        {cargando ? (
          ...
          <div className="table-responsive">
            <table className="table align-middle mb-0" style={{ minWidth: "1300px", backgroundColor: "#fffdf8" }}>
              <thead>
                <tr style={{ backgroundColor: ENCABEZADO }}>
                  {["ID", "Cód. Barras", "Producto", "Descripción", "Categoría", "Proveedor", "Usuario", "Rol", "Unidad", "Stock", "Precio", "Estado", "Ver"].map((h) => (
                    <th key={h} className={h === "ID" ? "ps-3" : h === "Ver" ? "text-center" : ""}
