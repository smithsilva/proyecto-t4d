...
export default function AsignacionTareas() {
  ...
  // Mecánicos encontrados al abrir el modal
  ...
  // =========================================
  // CARGAR MECÁNICOS POR ROL AL ABRIR MODAL
  // Consulta usuarios cuyo rol tenga
  // nombre_rol = 'Mecanico' (ajusta si difiere)
  // =========================================
// =========================================
// CARGAR MECÁNICOS POR ROL AL ABRIR MODAL
// =========================================
...
const abrirModal = async () => {
  ...
  if (rolError || !rolData) {
    console.error("No se encontró el rol Mecanico");
    setMecanicos([]);
  ...
  // =========================================
  // CREAR ASIGNACIÓN PARA TODOS LOS MECÁNICOS
  // Inserta un registro por cada mecánico activo
  // =========================================

  const crearAsignacion = async () => {
    if (!form.vehiculo.trim()) {
      alert("El vehículo es obligatorio.");
      return;
    }
    if (mecanicos.length === 0) {
      alert("No se encontraron mecánicos activos con ese rol.");
      return;
    }

    setGuardando(true);

    // Construye un insert por cada mecánico
   const payload = mecanicos.map((m) => ({
  ...
  estado: "Pendiente",
    ...
    if (err) {
      ...
      // Notificación por rol para cada mecánico
      const notificaciones = JSON.parse(localStorage.getItem("notificaciones")) || [];
      const nuevasNotifs = mecanicos.map((m) => ({
        id: Date.now() + m.id_usuario,
        rol: "Mecanico",
        id_usuario: m.id_usuario,
        titulo: "Nueva asignación",
        descripcion: `Nuevo trabajo de ${form.tipo_trabajo}`,
        vehiculo: form.vehiculo.trim(),
      ...
      localStorage.setItem(
        "notificaciones",
        JSON.stringify([...nuevasNotifs, ...notificaciones])
      );
  ...
  const filtradas = asignaciones.filter(
    (a) =>
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: "20px 22px", border: "1px solid #e5e7eb", overflowX: "auto" }}>
        ...
        {cargando && (
          <div style={{ textAlign: "center", padding: "40px 0" }}>
            ...
            <p style={{ marginTop: 12, color: "#9ca3af", fontSize: 13 }}>Conectando con Supabase...</p>
        ...
        {!cargando && error && (
          <div style={{ textAlign: "center", padding: "40px 0" }}>
            <p style={{ fontSize: 15, marginBottom: 6 }}>⚠️ No se pudo conectar a la base de datos</p>
            <p style={{ color: "#ef4444", fontSize: 12, marginBottom: 6, fontFamily: "monospace" }}>{error}</p>
            <p style={{ color: "#9ca3af", fontSize: 12, marginBottom: 16 }}>
              Verifica que <code>VITE_SUPABASE_URL</code> y <code>VITE_SUPABASE_ANON_KEY</code> estén en tu <code>.env</code>
            </p>
            <button onClick={cargarAsignaciones} style={{ padding: "8px 18px", borderRadius: 8, border: "1px solid #B89B6A", background: "#fff", color: "#B89B6A", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
              Reintentar conexión
            </button>
          </div>
        )}
                ...
                <tr>
                  <td colSpan={7} style={{ padding: "40px 0", textAlign: "center", color: "#9ca3af", fontSize: 13 }}>
                    {asignaciones.length === 0
                      ? "No hay asignaciones aún. Crea la primera con el botón \"Nueva Asignación\"."
      ...
      {modalNueva && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.45)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: 440, maxHeight: "90vh", overflowY: "auto" }}>
            <h5 style={{ fontWeight: 700, marginBottom: 4, fontSize: 16 }}>Nueva Asignación</h5>

            {/* Indicador de mecánicos que recibirán la tarea */}
            <div style={{ marginBottom: 20, padding: "10px 14px", borderRadius: 10, background: "#faf7f2", border: "1px solid #e8dcc8", display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ color: "#B89B6A" }}><UsersIcon /></span>
              {cargandoMec ? (
                <span style={{ fontSize: 12, color: "#9ca3af" }}>Buscando mecánicos activos...</span>
              ) : mecanicos.length > 0 ? (
                <span style={{ fontSize: 12, color: "#6b7280" }}>
                  Se asignará a <strong style={{ color: "#B89B6A" }}>{mecanicos.length} mecánico{mecanicos.length > 1 ? "s" : ""}</strong>:&nbsp;
                  {mecanicos.map((m) => m.username).join(", ")}
                </span>
              ) : (
                <span style={{ fontSize: 12, color: "#ef4444" }}>
  No se encontraron mecánicos activos
</span>
              )}
            </div>
            ...
            <textarea placeholder="Detalle del trabajo a realizar..." value={form.descripcion}
